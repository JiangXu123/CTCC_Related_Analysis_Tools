...
'''
This program is used for comparing two division matrices of the same cell, trying to find out the principle of chromosome organization.
...
def run(args):
    ...
    c4 = cooler.Cooler(cool_file_4)

    div_matrix_1_name = input("please input the name of the first division matrix, which will be shown on the graph:")
    div_matrix_2_name = input("please input the name of the second division matrix, which will be shown on the graph:")
    ...
    with open(division_pixel_comp_file, "w") as f2:
        ...
        csv_writer.writerow(['chr_1_name', 'chr_1_start', 'chr_1_end', 'chr_2_name', 'chr_2_start', 'chr_2_end', 'subcompartment', 'subcompartment_length', 'division_matrix_1_pixel_value', 'division_matrix_2_pixel_value', 'interaction_type'])  # write the header of the output file
        ...
        for k in range(0, len(chromosome_ls)):
            ...
            if not (chr_name in chrom_exclusion_ls):
                ...
                cis_4 = c4.matrix(sparse=False, balance=True).fetch(chromosome_ls[k])

                cis_div_matrix_1 = np.divide(cis_1, cis_2)
                cis_div_matrix_2 = np.divide(cis_3, cis_4)
                ...
                for r in range(0, cis_div_matrix_1.shape[0]):
                    for c in range(0, cis_div_matrix_1.shape[1]):
                        ...
                        cis_div_pixel_value_2 = cis_div_matrix_2[r, c]
                        ...
                        if (not np.isinf(cis_div_pixel_value_1)) & (not np.isnan(cis_div_pixel_value_1)) & (cis_div_pixel_value_1 != 0) & (not np.isinf(cis_div_pixel_value_2)) & (not np.isnan(cis_div_pixel_value_2)) & (cis_div_pixel_value_2 != 0):
                            ...
                            for comp in cis_comp_ls:
                                ...
                                csv_writer.writerow([cis_chr_1_name, cis_chr_1_start, cis_chr_1_end, cis_chr_2_name, cis_chr_2_start, cis_chr_2_end, comp, average_pixel_comp_len_dic[comp], cis_div_pixel_value_1, cis_div_pixel_value_2, 'cis'])
        ...
        for i in range(0, len(chromosome_ls)):  # generate the over-the-threshold trans pixel contents first
            for j in range(i + 1, len(chromosome_ls)):
                ...
                if (not (chr_name_1 in chrom_exclusion_ls)) & (not (chr_name_2 in chrom_exclusion_ls)):
                    ...
                    trans_4 = c4.matrix(sparse=False, balance=True).fetch(chromosome_ls[i], chromosome_ls[j])
                    trans_div_matrix_1 = np.divide(trans_1, trans_2)
                    trans_div_matrix_2 = np.divide(trans_3, trans_4)
                    ...
                    for y in range(0, trans_div_matrix_1.shape[0]):
                        for x in range(0, trans_div_matrix_1.shape[1]):
                            ...
                            trans_div_2_pixel_value = trans_div_matrix_2[y, x]
                            r_pixel_comp_len_dic = {"A1": 0, "A2": 0, "B1": 0, "B2": 0, "B3": 0, "B4": 0, "NA": 0}
                            c_pixel_comp_len_dic = {"A1": 0, "A2": 0, "B1": 0, "B2": 0, "B3": 0, "B4": 0, "NA": 0}
                            average_pixel_comp_len_dic = {"A1": 0, "A2": 0, "B1": 0, "B2": 0, "B3": 0, "B4": 0, "NA": 0}
                            if (not np.isinf(trans_div_1_pixel_value)) & (not np.isnan(trans_div_1_pixel_value)) & (trans_div_1_pixel_value != 0) & (not np.isinf(trans_div_2_pixel_value)) & (not np.isnan(trans_div_2_pixel_value)) & (trans_div_2_pixel_value != 0):
                                ...
                                for comp in trans_comp_ls:
                                    average_pixel_comp_len_dic[comp] = (r_pixel_comp_len_dic[comp] + c_pixel_comp_len_dic[comp]) / 2
                                    csv_writer.writerow([trans_chr_1_name, trans_chr_1_start, trans_chr_1_end, trans_chr_2_name, trans_chr_2_start, trans_chr_2_end, comp, average_pixel_comp_len_dic[comp], trans_div_1_pixel_value, trans_div_2_pixel_value, 'trans'])
                inter_time_end = time.perf_counter()
    ...
    div_2_cis_pixel_min = pixel_df.loc[pixel_df['interaction_type'] == 'cis'].describe().loc['min']['division_matrix_2_pixel_value']
    div_2_cis_pixel_max = pixel_df.loc[pixel_df['interaction_type'] == 'cis'].describe().loc['max']['division_matrix_2_pixel_value']
    if div_1_cis_pixel_max <= div_2_cis_pixel_max:
        div_cis_max = div_2_cis_pixel_max
    elif div_1_cis_pixel_max > div_2_cis_pixel_max:
        div_cis_max = div_1_cis_pixel_max
    ...
    div_2_trans_pixel_min = pixel_df.loc[pixel_df['interaction_type'] == 'trans'].describe().loc['min']['division_matrix_2_pixel_value']
    div_2_trans_pixel_max = pixel_df.loc[pixel_df['interaction_type'] == 'trans'].describe().loc['max']['division_matrix_2_pixel_value']
    if div_1_trans_pixel_max <= div_2_trans_pixel_max:
        div_tran_max = div_2_trans_pixel_max
    elif div_1_trans_pixel_max > div_2_trans_pixel_max:
        div_tran_max = div_1_trans_pixel_max
    ...
    print("All values are calculated without chrM and chrY")
    print(f"The highest non-inf cis value of the division matrix_2 is {div_2_cis_pixel_max}")
    print(f"The lowest non-zero cis value of the division matrix_2 is {div_2_cis_pixel_min}")
    print(f"The highest non-inf trans value of the division matrix_2 is {div_2_trans_pixel_max}")
    print(f"The lowest non-zero trans value of the division matrix_2 is {div_2_trans_pixel_min}")
    ...
    with open(aggr_comp_pixel_file, 'w') as f3:
        ...
        csv_writer.writerow(['compartment', 'interaction_type', 'threshold', 'div_mat_1_length', 'div_mat_2_length', 'total_length', 'div_1_to_total_ratio', 'div_2_to_total_ratio'])
        ...
        for cis_pixel_value_threshold in np.linspace(0, div_cis_max, 100):  # The value of each item in the division matrix is somewhere between 0 and positive infinity
            div_1_cis_comp_length_dic = {'A1': 0, 'A2': 0, 'B1': 0, 'B2': 0, 'B3': 0, 'B4': 0}
            div_2_cis_comp_length_dic = {'A1': 0, 'A2': 0, 'B1': 0, 'B2': 0, 'B3': 0, 'B4': 0}
            for compartment in cis_comp_ls:
                ...
                div_2_cis_comp_length_dic[compartment] = pixel_df.loc[(pixel_df['subcompartment'] == compartment) & (pixel_df['interaction_type'] == 'cis') & (pixel_df['division_matrix_2_pixel_value'] >= cis_pixel_value_threshold)]['subcompartment_length'].sum()
                csv_writer.writerow([compartment, 'cis', cis_pixel_value_threshold, div_1_cis_comp_length_dic[compartment], div_2_cis_comp_length_dic[compartment], comp_len_dic[compartment], round(100 * div_1_cis_comp_length_dic[compartment] / comp_len_dic[compartment], 5), round(100 * div_2_cis_comp_length_dic[compartment] / comp_len_dic[compartment], 5)])
        for trans_pixel_value_threshold in np.linspace(0, div_tran_max, 100):
            div_1_trans_comp_length_dic = {'A1': 0, 'A2': 0, 'B1': 0, 'B2': 0, 'B3': 0}
            div_2_trans_comp_length_dic = {'A1': 0, 'A2': 0, 'B1': 0, 'B2': 0, 'B3': 0}
            for compartment in trans_comp_ls:
                ...
                div_2_trans_comp_length_dic[compartment] = pixel_df.loc[(pixel_df['subcompartment'] == compartment) & (pixel_df['interaction_type'] == 'trans') & (pixel_df['division_matrix_2_pixel_value'] >= trans_pixel_value_threshold)]['subcompartment_length'].sum()
                csv_writer.writerow([compartment, 'trans', trans_pixel_value_threshold, div_1_trans_comp_length_dic[compartment],  div_2_trans_comp_length_dic[compartment], comp_len_dic[compartment], round(100 * div_1_trans_comp_length_dic[compartment] / comp_len_dic[compartment], 5), round(100 * div_2_trans_comp_length_dic[compartment] / comp_len_dic[compartment], 5)])
    ...
    fig, ax = plt.subplots(nrows=2, ncols=2, figsize=(16, 16), dpi=300)  # size of the plot is 16 inch wide and 8 inch height, encomparsing two picture horizontally
    fig.tight_layout(pad=5)  # make enough space between subplots
    sns.lineplot(x='threshold', y='div_1_to_total_ratio', hue='compartment', palette=["#D55E00", "#CC79A7", "#999999", "#56B4E9", "#F0E442"], linewidth=3, data=cis_df, ax=ax[0, 0])
    ax[0, 0].set_yscale('log')
    ax[0, 0].set_xlabel('Pixel Threshold', fontsize=20)
    ax[0, 0].set_ylabel('Enrichment Fold', fontsize=20)
    ax[0, 0].spines['right'].set_visible(False)
    ax[0, 0].spines['top'].set_visible(False)
    ax[0, 0].legend(frameon=False, fontsize=20)
    ax[0, 0].tick_params(labelsize=20)

    sns.lineplot(x='threshold', y='div_2_to_total_ratio', hue='compartment', palette=["#D55E00", "#CC79A7", "#999999", "#56B4E9", "#F0E442"], linewidth=3, data=cis_df, ax=ax[0, 1])
    ax[0, 1].set_yscale('log')
    ax[0, 1].set_xlabel('Pixel Threshold', fontsize=20)
    ax[0, 1].set_ylabel('Enrichment Fold', fontsize=20)
    ax[0, 1].spines['right'].set_visible(False)
    ax[0, 1].spines['top'].set_visible(False)
    ax[0, 1].legend(frameon=False, fontsize=20)
    ax[0, 1].tick_params(labelsize=20)

    sns.lineplot(x='threshold', y='div_1_to_total_ratio', hue='compartment', palette=["#D55E00", "#CC79A7", "#999999", "#56B4E9", "#F0E442"], linewidth=3, data=trans_df, ax=ax[1, 0])
    ax[1, 0].set_yscale('log')
    ax[1, 0].set_xlabel('Pixel Threshold', fontsize=20)
    ax[1, 0].set_ylabel('Enrichment Fold', fontsize=20)
    ax[1, 0].spines['right'].set_visible(False)
    ax[1, 0].spines['top'].set_visible(False)
    ax[1, 0].legend(frameon=False, fontsize=20)
    ax[1, 0].tick_params(labelsize=20)

    sns.lineplot(x='threshold', y='div_2_to_total_ratio', hue='compartment', palette=["#D55E00", "#CC79A7", "#999999", "#56B4E9", "#F0E442"], linewidth=3, data=trans_df, ax=ax[1, 1])
    ax[1, 1].set_yscale('log')
    ax[1, 1].set_xlabel('Pixel Threshold', fontsize=20)
    ax[1, 1].set_ylabel('Enrichment Fold', fontsize=20)
    ...
    fig.text(0.6, 0.98, div_matrix_2_name, fontsize=30)
...
def main():
    ...
    parser.add_argument("-c3", help="cool file on the top for the second division matrix", dest="cool_3", type=str, required=True)
    parser.add_argument("-c4", help="cool file on the bottom for the second division matrix", dest="cool_4", type=str, required=True)
