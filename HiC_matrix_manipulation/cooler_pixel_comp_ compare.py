...
## this program is used for calculating divsion matrix to find difference occured in different compartment between CTCC and HiC experiment
def run(args):
    ...
    # the first step is to calculate the how many pixels are same compartment pixel for a certain subcompartment
    ...
    with open(division_pixel_comp_file, "w") as f2:
        ...
        csv_writer.writerow(['chr_1_name', 'chr_1_start', 'chr_1_end', 'chr_2_name', 'chr_2_start', 'chr_2_end', 'subcompartment', 'threshold', 'interaction_type'])  # write the header of the output file
        for k in range(0, len(chromosome_ls)):
            ...
            for r in range(0, cis_div_matrix.shape[0]):
                for c in range(0, cis_div_matrix.shape[1]):
                    ...
                    cis_pixel_value = cis_div_matrix[r, c]  # r, c are the row and column of the pixel in the individual trans matrix
                    if (not np.isinf(cis_pixel_value)) & (not np.isnan(cis_pixel_value)) & (cis_pixel_value != 0):
                        ...
                        try:
                            ...
                            c_comp = "c_comp_nv"
                        if r_comp == c_comp:
                            first_two_digits = float(str(cis_pixel_value).split('.')[0] + '.' + str(cis_pixel_value).split('.')[1][0])
                            csv_writer.writerow([chromosome_ls[k], chromosome_ls[k], r_comp, first_two_digits, 'cis'])
                    ...
                    for x in range(0, trans_div_matrix.shape[1]):
                        ...
                        if (not np.isinf(trans_pixel_value)) & (not np.isnan(trans_pixel_value)) & (trans_pixel_value != 0):
                            ...
                            if chr_1_comp == chr_2_comp:
                                first_two_digits = float(str(trans_pixel_value).split('.')[0] + '.' + str(trans_pixel_value).split('.')[1][0])
                                csv_writer.writerow([bin_dic[abs_pixel_bin_y][0], bin_dic[abs_pixel_bin_x][0], chr_1_comp, first_two_digits, 'trans'])
                inter_time_end = time.perf_counter()
    ...
    cis_pixel_min = pixel_df.loc[pixel_df['interaction_type'] == 'cis'].describe().loc['min']['threshold']
    cis_pixel_max = pixel_df.loc[pixel_df['interaction_type'] == 'cis'].describe().loc['max']['threshold']
    trans_pixel_min = pixel_df.loc[pixel_df['interaction_type'] == 'trans'].describe().loc['min']['threshold']
    trans_pixel_max = pixel_df.loc[pixel_df['interaction_type'] == 'trans'].describe().loc['max']['threshold']
    print(f"The highest non-inf cis value of the division matrix is {cis_pixel_max}")
    print(f"The lowest non-zero cis value of the division matrix is {cis_pixel_min}")
    print(f"The highest non-inf trans value of the division matrix is {trans_pixel_max}")
    print(f"The lowest non-zero value of the division matrix is {trans_pixel_min}")
    ...
    with open(aggr_comp_pixel_file, 'w') as f3:
        ...
        for cis_pixel_value_threshold in np.arange(0, cis_pixel_max, 0.1):
            cis_comp_count_dic = {'A1': 0, 'A2': 0, 'B1': 0, 'B2': 0, 'B3': 0, 'B4': 0}
            for compartment in comp_ls:
                cis_comp_count_dic[compartment] = pixel_df.loc[(pixel_df['subcompartment'] == compartment) & (pixel_df['interaction_type'] == 'cis') & (pixel_df['threshold'] >= cis_pixel_value_threshold)].count()['interaction_type']
        ...
        for trans_pixel_value_threshold in np.arange(0, trans_pixel_max, 0.1):
            trans_comp_count_dic = {'A1': 0, 'A2': 0, 'B1': 0, 'B2': 0, 'B3': 0, 'B4': 0}
            for compartment in comp_ls:
                trans_comp_count_dic[compartment] = pixel_df.loc[(pixel_df['subcompartment'] == compartment) & (pixel_df['interaction_type'] == 'trans') & (pixel_df['threshold'] >= trans_pixel_value_threshold)].count()['interaction_type']
                ...
                if compartment != 'B4':
                    csv_writer.writerow([compartment, 'trans', trans_pixel_value_threshold, trans_comp_count_dic[compartment], trans_pixel_total_counts_dic[compartment], round(100*trans_comp_count_dic[compartment]/trans_pixel_total_counts_dic[compartment], 2)])
