#! /usr/bin/env python
# This code is used to calculate aggregated breaking point information from CTCC or HiC experiment, the input bam file
# was trimmed off the connector sequence using the cutdapt program
import pysam
import time
import argparse
import pandas as pd
    ...
    for line in bf.fetch(until_eof=True):
        if line.mapq >= mq:
            ...
            if 'X' in line.cigarstring:
                ...
                if trim_sel == 3:   # if 3' connector was trimmed
                    if line.flag == 0:  # if the 3' trimmed sequence mapped to forward strand
                        df = pd.DataFrame([[line.reference_name, line.reference_start + line.cigar[0][1], count]],
                                  columns=['RNAME', 'start', 'count'])
                    if line.flag == 16:  # if the 3' trimmed sequence mapped to reverse strand
                        df = pd.DataFrame([[line.reference_name, line.reference_start, count]],
                                  columns=['RNAME', 'start', 'count'])
                    new_df = pd.concat([new_df, df], sort=False)
                if trim_sel == 5:  # if 5' connector was trimmed
                    if line.flag == 0:  # if the 5' trimmed connector mapped to forward strand
                        df = pd.DataFrame([[line.reference_name, line.reference_start, count]],
                                  columns=['RNAME', 'start', 'count'])
                    if line.flag == 16:
                        df = pd.DataFrame([[line.reference_name, line.reference_start + line.cigar[0][1], count]],
