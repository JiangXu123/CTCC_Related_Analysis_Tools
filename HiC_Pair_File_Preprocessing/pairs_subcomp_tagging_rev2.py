#! /usr/bin/env python
# this is the revision of the program 'pairs_subcomp_tagging_rev1.py' enabling multi-processing of other
# forms of pair data
import os
    ...
    with open(in_file, 'r') as file1:
        with open(out_file, 'w') as file2:
            ...
            for line in csv_reader:
                try:
                    result1 = tb.query(line[0], int(line[1]) - 1, int(line[1]))
                    result2 = tb.query(line[2], int(line[3]) - 1, int(line[3]))
                    read1_comp = next(result1)[3]
                    read2_comp = next(result2)[3]
                    interaction_type = read1_comp + '-' + read2_comp
                    if line[0] == line[2]:
                        distance = abs(int(line[3]) - int(line[1]))
                    elif line[0] != line[2]:
                        distance = 'NA'
                    csv_writer.writerow([line[0], line[1], 'NA', line[2], line[3], 'NA', distance, interaction_type, experiment])
                    # columns are: chr_1_name, chr_1_pos, chr_2_name, chr_2_pos, distance, interaction_type, experiment
                except:
                    pass
