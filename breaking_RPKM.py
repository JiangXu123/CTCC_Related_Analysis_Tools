        ...
        for chromosome in chrom_ls:
            for i in range(0, chrom_dic[chromosome], bin_size):
                ...
                try:
                    ...
                    for item in result:
                        temp_ls.append(item)
                    csv_writer1.writerow([chromosome, i, end, bin_count, exp, temp_ls[0][3][:2], round(1000000*len(temp_ls)/total_breaks, 4)])
                except:
                    csv_writer1.writerow([chromosome, i, end, bin_count, exp, compartment, 0])
                temp_ls = []
                bin_count += 1
    end = time.perf_counter()
    print(f'{total_reads} breaking position processed in {round(end - start, 2)} sec')
