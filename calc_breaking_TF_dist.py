...
# and calculate the distance to the center of TF ChIP-Seq sites(location)
# aggregate the breaking distance to TF peak center
...
def run(args):
    ...
    try:
        os.makedirs('./aggr_dist_file')
        print('Folder for aggregated distance file created at aggr_dist_file')
    except FileExistsError:
        print(f'Folder aggr_dist_file already exists, will use this folder')    

    bin_name = get_bin_name(indexed_bin_file, tf_list)
    temp_path = os.path.abspath('.') + '/' + bin_name + '_temp'

    for file in input_file_name_ls:
        input_file_path = os.path.abspath(input_folder) + '/' + file
        input_file_path_ls.append(input_file_path)
        dist_file_path = './' + file.split('.')[0] + '_' + bin_name + '_dist.tsv'
        # the output_file_path is the path of intermediate output_file contain breaking_dist_to_ref information
        dist_file_path_ls.append(dist_file_path)
    ...
    for i in range(0, len(input_file_path_ls)):
        ...
        block_num = len(os.listdir(temp_path))  # block_num contains the actual split file number
        ...
        processes = []
        temp_in_file_name_ls = os.listdir(temp_path)
        temp_in_file_path_ls = []
        temp_out_file_path_ls = []
        for name in temp_in_file_name_ls:
            temp_in_file_path_ls.append(temp_path + '/' + name)
            temp_out_file_path_ls.append(temp_path + '/temp_out_' + name.split('_')[-1])
        ...
        with open(dist_file, 'w') as file1:  # concatenate the temporary output files
        ...
        aggregate_nc_dist(dist_file, aggr_output_file, experiment, bin_name, bin_radius)
        os.unlink(dist_file)  # remove the intermediate 'breaking_dist_to_ref' file
    ...
    final_aggr_file = './aggr_dist_file/' + bin_name + '_' + str(bin_radius) + '_bin' + '.tsv'
    with open(final_aggr_file, 'w') as file3:
        for aggr_file in aggr_output_file_path_ls:
            with open(aggr_file, 'r') as file4:
                for line in file4:
                    file3.write(line)
    for aggr_file in aggr_output_file_path_ls:   # deleted individual aggregated files
        os.unlink(aggr_file)
    end = time.perf_counter()
    print(f'{len(input_file_name_ls)} file(s) processed within {round(end - start, 2)} sec')


def get_bin_name(indexed_bin_file, tf_list):
    if not tf_list:     # if NO transcription factor(TF) reference list was set
        bin_name = indexed_bin_file.split('/')[-1].split('.')[0] # bin_name is inferred from index file name
        return bin_name

    TF_dic = {}
    if tf_list:  # if transcription factor(TF) reference list was set
        with open(tf_list, 'r') as file:
            csv_reader = csv.reader(file, delimiter='\t')
            for line in csv_reader:
                TF_dic.update({line[0]: line[1]})   # the first column of TF list is the number, the second column is the TF name
        bin_name = TF_dic[indexed_bin_file.split('/')[-1].split('_')[0]]
        return bin_name
        ...
        with open(out_file, 'w') as aggregated_data:
            ...
            for line in csv_reader:
                try:
                    ...
                    item = next(result)
                    distance = int(line[2]) - int(item[3])  # item[4] contains the bin center position
                    if abs(distance) <= bin_radius:
                        csv_writer.writerow([exp, int(item[4]), line[3], line[0], distance])
        ...
        with open(aggr_output_file, 'w') as aggregated_data:
            ...
            for compartment in compartment_ls:
                for i in range(-bin_radius, bin_radius+1):  # i is the relative distance to the center of the peak in bp
                    if compartment == 'A1':
                        if A1_bin_count == 0:
                            csv_writer.writerow([experiment, compartment, i, A1_dic[i], A1_bin_count, line_number, 'NaN', ref_bins_name])
                        else:
                    ...
                    if compartment == 'A2':
                        if A2_bin_count == 0:
                            csv_writer.writerow([experiment, compartment, i, A2_dic[i], A2_bin_count, line_number, 'NaN', ref_bins_name])
                        else:
                    ...
                    if compartment == 'B1':
                        if B1_bin_count == 0:
                            csv_writer.writerow([experiment, compartment, i, B1_dic[i], B1_bin_count, line_number, 'NaN', ref_bins_name])
                        else:
                    ...
                    if compartment == 'B2':
                        if B2_bin_count == 0:
                            csv_writer.writerow([experiment, compartment, i, B2_dic[i], B2_bin_count, line_number, 'NaN', ref_bins_name])
                        else:
                    ...
                    if compartment == 'B3':
                        if B3_bin_count == 0:
                            csv_writer.writerow([experiment, compartment, i, B3_dic[i], B3_bin_count, line_number, 'NaN', ref_bins_name])
                        else:
                    ...
                    if compartment == 'B4':
                        if B4_bin_count == 0:
                            csv_writer.writerow([experiment, compartment, i, B4_dic[i], B4_bin_count, line_number, 'NaN', ref_bins_name])
                        else:
            ...
            # column names are:
            # experiment_name, compartment, position_to_nc_center, position_count, compartment_bin_count
...
def main():
    ...
    parser.add_argument("-l", help="TF name reference file", dest="tf_l", type=str, required=False)
